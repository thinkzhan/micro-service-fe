                        ...
                        template(
                           v-if="column.action"
                            ...
                            template(v-for="ac in column.action")
                                el-button(v-if="ac==='update'" type="primary" plain size="small"
                                    @click="showDialog({data: scope.row, update: true, title:'编辑'})") 编辑
                                el-button(v-if="ac==='delete'" type="primary" plain size="small"
...
export default {
    ...
    async created() {
        ...
        this.searchConfig.data &&
            this.searchConfig.data.forEach(d => {
                if (d.cache) {
                    this.cacheModelList[d.model] = true;
                    const cache = localStorage.getItem(
                        `${this.$route.path}--${d.model}`
                    );
    ...
    methods: {
        ...
        onSearchChange(data, conf) {
            conf.defaultAction === false
                ? this.$emit(`${conf.type}Change`, conf)
                : this.searchClick();
        },
