/** *** 弹窗编辑 *** 1、简单表单编辑配合FormMaker dialog-maker(
:config="dialogConfig" :formConfig="formConfig" // formConfig结构见FormMaker
@update="" @add="" ) *** 2、slot自定义，slot优先级高于formConfig dialog-maker(
:config="dialogConfig" ) template(slot-scope="scope") div
{{ scope.dialogConfig }}
// div 不需要scope **/
    ...
    methods: {
        submitForm() {
            this.$refs.FormMaker.$refs['form'].validate(valid => {
                if (valid) {
                    if (
                        this.formConfig.rule &&
                        this.formConfig.rule.beforeSubmitForm
                    ) {
