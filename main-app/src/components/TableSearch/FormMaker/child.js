    ...
    if (typeof item.list === 'function') {
        ...
        if (item.selectType === 'idName') {
            return item.list.map(item => {
                ...
                if (
                    item.vIfList &&
                    !item.vIfList(self.formModel, item.name, item.id)
                ) {
                    return null;
                }
                return h(
                    `el-${item.type}`,
                    {
                        props: {
                            ...
                            label: item.id,
                            disabled:
                                typeof item.disabledList === 'function'
                                    ? item.disabledList(
                                          self.formModel,
                                          item.name,
                                          item.id
            ...
            return Object.keys(item.list).map(key => {
                ...
                if (
                    item.vIfList &&
                    !item.vIfList(self.formModel, item.list[key], key)
                ) {
                    return null;
                }
                return h(
            ...
            scopedSlots: {
                default: scope =>
                    h('span', {
                        domProps: {
                            innerHTML: v.format
                                ? v.format(scope.row[v.prop], scope.row)
                                : scope.row[v.prop]
                        }
                    })
            }
